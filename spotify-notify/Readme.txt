BEFORE YOU INSTALL THIS:

1. Install spotify for linux (Native, not wine.)
2. Edit the .desktop file with a text editor (gedit, vim, nano...) and change the
   "Exec=" and "Icon=" path to direct to whereever you installed your\
   spotify-notify.py file. 

TO INSTALL

Install the edited .desktop file with the command:

desktop-file-install /path/to/spotify.desktop

